...
def ConvertGrid(gridfilename):
    ...
    datalabel = gridfilename.split(".")[0]

    for line in gridfile:
        if line.startswith('--') or not line.strip() or line.startswith('INCLUDE'):
            ...
            ydim = int(ydim)
            zdim = int(zdim)
        elif line.split(".")[0].split()[0].split("_")[1] == "COORD":
            coords = ReadSection(open(line.split()[0].replace("\'","")))
        ...
        elif line.split(".")[0].split()[0].split("_")[1] == "ZCORN":
            zcorn = ReadSection(gridfile)
            ug = CreateVTKCells(ug, xdim, ydim, zdim)
            ug.SetPoints(CreateVTKPoints(xcoords, ycoords, zcorn))

        elif line.split(".")[0].split()[0].split("_")[1] == "ACTNUM":
            #ug = ReadScalarSection('ACTNUM', gridfile, ug)
            print('ACTNUM')
            
        elif line.split(".")[0].split()[0].split("_")[1] == "PROP":
            props.append(str(line.split(".")[0].split()[0].split("_")[2:]).strip("[]").replace(",","").replace("\'",""))
            #ug = ReadScalarSection(line,gridfile,ug)
